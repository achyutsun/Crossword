...
public class Lexicon
{
	...
	private class IndexEntry
	{
		...
		{
			iPrefix = new String(aEntry.iPrefix);
	...
	private StringBuffer iBuffer = new StringBuffer(MAX_LENGTH);
	private int iPos = 0;
	// String iLetterSet;
	// boolean iUseAll;
	// int iMode = 0;
	...
	boolean check(String aCandidate, int aDictionary, int aDictMask, boolean aFuzzy)
	{
		...
		{
			/*
			 * String buffer; String word; i2Letters.Seek(ESeekStart,startPos);
			 * do { i2Letters.Read(buffer); if (buffer.Length()<3) return
			 * EFalse; word.Copy(buffer.Left(2)); } while
			 * (word.CompareF(aCandidate)<0); aDictionary=buffer[2]; return
		...
		{
			// fuzzy search
			iSearchContext.iLetterSet = new String(aCandidate.toLowerCase());
